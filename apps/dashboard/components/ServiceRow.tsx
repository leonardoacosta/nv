...
export default function ServiceRow({ service, lastChecked }: ServiceRowProps) {
  ...
  return (
    <div className="rounded-md border border-transparent hover:border-ds-gray-alpha-200 transition-colors">
      ...
      >
        ...
        {/* Status dot */}
        <span
          className={`inline-block w-1.5 h-1.5 rounded-full shrink-0 ${STATUS_DOT[service.status]}`}
      ...
      {expanded && (
        <div className="px-3 pb-3 pt-1 ml-9 space-y-2 border-t border-ds-gray-alpha-200">
          ...
          <div className="flex items-center gap-2">
            <span className="text-label-12 text-ds-gray-700 w-12">URL</span>
          ...
          {/* Latency / last check */}
          <div className="flex items-center gap-2">
            <span className="text-label-12 text-ds-gray-700 w-12">Check</span>
            <span className="text-copy-13 text-ds-gray-900 font-mono">
              {lastChecked ? formatRelativeTime(lastChecked) : "never"}
            </span>
          </div>
          ...
          {service.tools.length > 0 && (
            <div className="flex items-start gap-2">
              <span className="text-label-12 text-ds-gray-700 w-12 pt-0.5">
                Tools
              </span>
