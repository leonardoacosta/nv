...
//! * `query_ado_work_items(...)` — query work items via WIQL.
//!
...
impl AdoClient {
    ...
    /// Create a client using an AAD Bearer token instead of PAT (optional enhancement).
    pub fn from_aad_token(org: &str, token: &str) -> Result<Self> {
        ...
        headers.insert(
            ...
            format!("Bearer {token}")
                .parse()
                .map_err(|_| anyhow!("invalid AAD token value"))?,
        );
    ...
    pub async fn work_items_by_wiql(
        ...
        let wiql_resp = self.http.post(&wiql_url).json(&body).send().await?;
        ...
        ToolDefinition {
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    ...
                    "assigned_to": {
                        "type": "string",
                        "description": "Filter by assignee — use '@Me' for the authenticated PAT user (default: '@Me')."
                    },
                    "state": {
                        "type": "string",
                        "description": "Filter by state: 'active', 'new', 'resolved', or 'all' (default: 'active').",
...
pub fn format_work_items(project: &str, items: &[AdoWorkItem]) -> String {
    ...
    let mut lines = vec![format!("Work Items — {project} ({active_count} active)")];
...
pub async fn query_ado_work_items(
    ...
    let items = client
        .work_items_by_wiql(&project_name, &wiql, limit)
        .await?;
...
mod tests {
    ...
    fn test_build_wiql_all_states() {
        ...
        // "all" has no state filter in the WHERE clause (no = or <> on State)
