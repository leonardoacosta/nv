...
/// Connect to a project's Neon database — public entry point for health checks.
///
/// Wraps the internal `connect` function so callers (e.g. `Checkable` impls
/// in the daemon) can obtain a client without accessing private internals.
pub async fn connect_for_check(project: &str) -> Result<tokio_postgres::Client> {
    connect(project).await
}
...
pub fn format_projects(projects: &[ProjectSummary]) -> String {
    ...
    for p in projects {
        let created = super::relative_time(&p.created_at);
...
pub fn format_endpoints(endpoints: &[EndpointSummary]) -> String {
    ...
    for e in endpoints {
        ...
        let last_active = super::relative_time(&e.last_active);
