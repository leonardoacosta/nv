...
/// Connect to cortex-postgres — public entry point for health checks.
///
/// Only exposes the connection itself; callers are responsible for running
/// their own health probes (e.g. `SELECT 1`).
pub async fn connect_for_check() -> Result<tokio_postgres::Client> {
    connect().await
}
