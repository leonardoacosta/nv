...
- [x] [3.1] [P-1] Implement `Checkable` for StripeClient — read: `GET /v1/balance`, write: `POST /v1/invoices` empty body [owner:general-purpose]
- [x] [3.2] [P-1] Implement `Checkable` for VercelClient — read: `GET /v13/user`, write: returns None [owner:general-purpose]
- [x] [3.3] [P-1] Implement `Checkable` for SentryClient — read: `GET /api/0/` org info, write: returns None [owner:general-purpose]
- [x] [3.4] [P-1] Implement `Checkable` for NeonClient — read: `SELECT 1`, write: returns None [owner:general-purpose]
- [x] [3.5] [P-1] Implement `Checkable` for PosthogClient — read: `GET /api/projects/`, write: returns None [owner:general-purpose]
- [x] [3.6] [P-1] Implement `Checkable` for UpstashClient — read: `INFO` command, write: returns None [owner:general-purpose]
- [x] [3.7] [P-1] Implement `Checkable` for ResendClient — read: `GET /domains`, write: returns None [owner:general-purpose]
- [x] [3.8] [P-1] Implement `Checkable` for AdoClient — read: `GET /_apis/projects`, write: returns None [owner:general-purpose]
- [x] [3.9] [P-1] Implement `Checkable` for HaClient — read: `GET /api/`, write: `POST /api/services/light/turn_on` empty [owner:general-purpose]
- [x] [3.10] [P-1] Implement `Checkable` for DockerClient — read: `docker info`, write: returns None [owner:general-purpose]
- [x] [3.11] [P-1] Implement `Checkable` for PlaidClient — read: `SELECT 1`, write: returns None [owner:general-purpose]
- [x] [3.12] [P-1] Implement `Checkable` for GithubClient — read: `gh auth status`, write: returns None [owner:general-purpose]
- [x] [3.13] [P-1] Implement `Checkable` for CloudflareClient — read: `GET /user/tokens/verify`, write: returns None [owner:general-purpose]
- [x] [3.14] [P-1] Implement `Checkable` for DopplerClient — read: `GET /v3/me`, write: returns None [owner:general-purpose]
- [x] [3.15] [P-2] Convert all `Option<XClient>` in SharedDeps to `Option<ServiceRegistry<XClient>>` [owner:general-purpose]
- [x] [3.16] [P-2] Update `main.rs` — build `ServiceRegistry` for each service from config + env vars [owner:general-purpose]
- [x] [3.17] [P-2] Update `execute_tool()` — resolve client from registry by project param where applicable [owner:general-purpose]
- [x] [3.18] [P-2] Update multi-instance config in `config/nv.toml` with commented examples for all services [owner:general-purpose]
