...
- [x] [2.1] Unit test: fresh database migration -- create temp DB, run `MessageStore::init()`, verify all tables exist via `SELECT name FROM sqlite_master WHERE type='table'` [owner:api-engineer]
- [x] [2.2] Unit test: idempotent migration -- run `MessageStore::init()` twice on same DB file, verify no error and schema unchanged [owner:api-engineer]
- [x] [2.3] Unit test: PRAGMA user_version tracks migration count -- after `MessageStore::init()`, query `PRAGMA user_version` and assert it equals the number of migrations (currently 4) [owner:api-engineer]
- [x] [2.4] Unit test: ScheduleStore migration -- create temp DB, run `ScheduleStore::new()`, verify schedules table exists [owner:api-engineer]

### Verify

- [x] [3.1] `cargo build` passes for all workspace members [owner:api-engineer]
- [x] [3.2] `cargo test -p nv-daemon` -- all new and existing tests pass [owner:api-engineer]
- [x] [3.3] `cargo clippy -- -D warnings` passes [owner:api-engineer]
