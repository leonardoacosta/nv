...
- [x] [2.1] Add `#[tokio::test] #[ignore]` smoke test `persistent_subprocess_smoke` that spawns a
...
- [x] [2.2] Remove `tools_json: Option<String>` field from `SpawnConfig`; remove the block in
  `spawn_persistent` that appends `--tools-json` and the serialized tools JSON to `base_args`
  [owner:api-engineer]
- [x] [2.3] Remove `PersistentSession::new` parameter logic that serializes tools into
...
- [x] [2.4] Add `"--no-mcp".into()` to `base_args` in `spawn_persistent`, immediately after the
  `--no-session-persistence` entry; log a `tracing::debug!` line confirming the flag is present
  [owner:api-engineer]
- [x] [2.5] In `drain_init_events`, change `Duration::from_secs(10)` to `Duration::from_secs(20)`;
  update the `tracing::warn!` message to say "20s timeout" [owner:api-engineer]
- [x] [2.6] Verify the smoke test from [2.1] passes against the development `claude` binary with
  ...
  [owner:api-engineer]
- [x] [2.7] In `PersistentSession::new`, change `fallback_only: true` to `fallback_only: false`;
...
- [x] [2.8] Add unit test `fallback_reset_after_cooldown`: construct `SessionInner` directly with
...
- [x] [2.9] Add unit test `fallback_no_reset_within_cooldown`: same setup but
...
- [x] [3.4] `cargo test -p nv-daemon` passes (all non-ignored tests, including new FALLBACK_RESET
