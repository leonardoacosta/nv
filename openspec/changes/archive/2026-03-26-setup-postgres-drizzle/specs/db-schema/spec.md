# Capability: Postgres + Drizzle DB Package

## Why

The Nova TS daemon needs a shared, strongly-typed data layer. SQLite served the Rust daemon well,
but the upcoming TS daemon plus dashboard require pgvector for semantic search and a package that
can be imported by multiple consumers. This spec adds the Postgres infrastructure and the
`packages/db/` Drizzle package that fulfils that need.
