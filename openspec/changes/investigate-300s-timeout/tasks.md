...
- [ ] Add `is_edit_reply: bool` field to `WorkerTask` in `crates/nv-daemon/src/worker.rs` —
  defaults to `false`; set to `true` in `process_trigger_batch` when `editing_action_id` is
  `Some` at dispatch time [owner:api-engineer]
- [ ] Add `timeout_reason` string field to the `tracing::warn!` span in the primary timeout
  branch (`worker.rs` line ~319) — value `"edit_wait"` if `task.is_edit_reply`, else
  `"active_work"` [owner:api-engineer]
- [ ] Add `timeout_reason` field to the queued-worker timeout branch (`worker.rs` line ~404)
  with the same logic [owner:api-engineer]
- [ ] Add `stage_elapsed_ms = task_start.elapsed().as_millis()` to both timeout warn spans so
  actual elapsed time is visible in logs [owner:api-engineer]
- [ ] Add `is_edit_reply` to the `"worker started"` tracing span so the log entry identifies
  edit-reply workers from the start [owner:api-engineer]

### Phase 2: Edit flow — consume `editing_action_id`

- [ ] In `Orchestrator::process_trigger_batch` (`orchestrator.rs`), add a block before worker
...
- [ ] Add `editing_action_id: Option<Uuid>` field to `WorkerTask` in `worker.rs` [owner:api-engineer]
- [ ] In `Worker::run` (`worker.rs`), at context-build time, check `task.editing_action_id`:
...
- [ ] After Claude's response in the edit-aware session, update the `PendingAction` payload
...
- [ ] Clear `editing_action_id` in `Orchestrator` after dispatch (`.take()` handles this
  automatically; verify it is not re-set on the same batch) [owner:api-engineer]

### Phase 3: Extended timeout for Edit-reply tasks

- [ ] In `WorkerPool::spawn_worker` (`worker.rs`), compute `effective_timeout_secs`:
  ...
  [owner:api-engineer]
- [ ] Add `tracing::info!(effective_timeout_secs, is_edit_reply, "worker timeout configured")`
  at dispatch so the extended timeout is logged [owner:api-engineer]
- [ ] Change the Telegram timeout message for edit-reply tasks to:
...
- [ ] In the primary timeout branch (`worker.rs` line ~329), remove `let _ = chat_id;` and
  add `chat_id: task_tg_chat_id` to the `tracing::warn!` span so the chat ID is logged
  [owner:api-engineer]
- [ ] In the same branch, pass `task_tg_chat_id` into the `OutboundMessage` routing — if
...
- [ ] Apply the same fix to the queued-worker timeout branch (`worker.rs` line ~413)
  [owner:api-engineer]

### Phase 5: "Still working" feedback

- [ ] In `WorkerPool::spawn_worker`, before the `tokio::time::timeout` wrapper, add a
...
- [ ] Ensure the warning future is cancelled if `Worker::run` completes before it fires —
  use `tokio::select!` with a `break`/`return` path on worker completion [owner:api-engineer]
- [ ] Do not send the "still working" message for `is_edit_reply` tasks — those are waiting on
  Leo, not on Claude [owner:api-engineer]

### Unit Tests

- [ ] Add unit test: `WorkerTask` with `is_edit_reply = true` sets `timeout_reason =
  "edit_wait"` in the emitted `WorkerEvent::Error` log (mock the event channel)
  [owner:api-engineer]
- [ ] Add unit test: `editing_action_id.take()` in `process_trigger_batch` correctly consumes
...
- [ ] `cargo test -p nv-daemon` passes [owner:api-engineer]
