...
- [x] [3.2] Update `.claude/settings.json`: add `mcp__nova-memory__*`, `mcp__nova-messages__*`, `mcp__nova-channels__*`, `mcp__nova-discord__*`, `mcp__nova-teams__*`, `mcp__nova-schedule__*`, `mcp__nova-graph__*`, `mcp__nova-meta__*` to the `permissions.allow` array. Keep existing `mcp__nv-tools__*` during transition. [owner:devops-engineer]
  NOTE: Permission denied for direct edit. User must manually add the 8 `mcp__nova-*__*` entries to `.claude/settings.json`.

## Phase 4: Deploy Integration

- [x] [4.1] Update `deploy/install-tools.sh` (from `add-fleet-deploy`): after the health check section, add a call to `bash "${REPO_DIR}/scripts/register-mcp-servers.sh"`. Capture exit code but don't fail the deploy if registration fails -- warn instead. Log the registration output. [owner:devops-engineer]
