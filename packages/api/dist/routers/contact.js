...
export const contactRouter = createTRPCRouter({
    ...
    update: protectedProcedure
        ...
        if (input.channel_ids !== undefined)
            updates.channelIds = input.channel_ids;
        if (input.relationship_type !== undefined)
            updates.relationshipType = input.relationship_type;
    ...
    getRelated: protectedProcedure
        ...
        const messagesByChannel = await Promise.all(channelPairs.map(([channel, senderId]) => db
            .select()
            .from(messages)
            .where(and(eq(messages.channel, channel), eq(messages.sender, senderId)))
        ...
        // Total message count
        const messageCounts = await Promise.all(channelPairs.map(([channel, senderId]) => db
            .select({ total: count() })
            .from(messages)
            .where(and(eq(messages.channel, channel), eq(messages.sender, senderId)))));
        const messageCount = messageCounts.reduce((sum, rows) => sum + (rows[0]?.total ?? 0), 0);
        // Related obligations via channel + time proximity
        let relatedObligations = [];
        if (channelsActive.length > 0 && allMessages.length > 0) {
            const obligationCandidates = await Promise.all(channelsActive.map((channel) => db
                .select()
                .from(obligations)
            ...
            const filtered = flatCandidates.filter((obl) => {
                ...
                return messageTimestamps.some((ts) => Math.abs(ts - oblTime) <= ONE_HOUR_MS);
            });
        ...
        if (peopleTopic) {
            ...
            const matched = sections.find((s) => s.toLowerCase().includes(contact.name.toLowerCase()));
    ...
    `);
        const totalMessages = await db
            .execute(sql `SELECT COUNT(*)::int AS count FROM ${messages}`)
            .then((r) => r[0]?.count ?? 0);
    ...
    resolve: protectedProcedure
        ...
        for (const row of rows) {
            const channelIds = row.channelIds;
            if (!channelIds)
                continue;
