import { and, count, desc, eq, ilike, inArray, like, max, } from "drizzle-orm";
import { z } from "zod";
...
export const projectRouter = createTRPCRouter({
    ...
    list: protectedProcedure
        ...
        if (projectRows.length === 0) {
            ...
            if (envProjects) {
                try {
                    seedList = JSON.parse(envProjects);
                }
                catch {
        ...
        const obligationActive = await db
            ...
            .from(obligations)
            .where(and(inArray(obligations.projectCode, codes), inArray(obligations.status, ACTIVE_OBLIGATION_STATUSES)))
            .groupBy(obligations.projectCode);
        ...
        for (const row of obligationTotals) {
            if (row.projectCode)
                totalByCode.set(row.projectCode, row.total);
        }
    ...
    extract: protectedProcedure.mutation(async () => {
        ...
        if (allProjects.length === 0) {
            return { projects_updated: 0, sources_scanned: [] };
        }
        ...
        for (const project of allProjects) {
            ...
            const [messageStats, sessionStats, obligationStats, memoryTopics, diaryStats, topSenders] = await Promise.all([
                db
                    .select({
            ...
            if (messageStats.messageCount > 0)
                sourcesSet.add("messages");
            if (sessionStats.sessionCount > 0)
                sourcesSet.add("sessions");
            if (obligationStats.length > 0)
                sourcesSet.add("obligations");
            if (diaryStats.diaryCount > 0)
                sourcesSet.add("diary");
            const relevantMemory = memoryTopics.filter((t) => t.content.toLowerCase().includes(code.toLowerCase()));
            if (relevantMemory.length > 0)
                sourcesSet.add("memory");
            const oblSummary = {};
