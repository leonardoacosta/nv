...
function deriveService(command) {
...
export const sessionRouter = createTRPCRouter({
    ...
    analytics: protectedProcedure.query(async () => {
        ...
        const [todayResult, totalResult, avgDurationResult, daily7dResult, projectResult] = await Promise.all([
            db
                .select({ total: count() })
                .from(sessions)
            ...
            db
                .select({
                avg: sql `AVG(EXTRACT(EPOCH FROM (${sessions.stoppedAt} - ${sessions.startedAt})) / 60)`,
            })
                .from(sessions)
                .where(sql `${sessions.stoppedAt} IS NOT NULL`),
            db
                .select({
                date: sql `DATE(${sessions.startedAt} AT TIME ZONE 'UTC')`,
                total: count(),
            })
                .from(sessions)
