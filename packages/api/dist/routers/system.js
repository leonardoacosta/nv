...
function getObligationSeverity(status) {
    ...
    if (lower.includes("failed") || lower.includes("error"))
        return "error";
    if (lower === "open" || lower.includes("detected"))
        return "warning";
    return "info";
}
export const systemRouter = createTRPCRouter({
    ...
    config: protectedProcedure.query(() => {
        return {
            tool_router_url: process.env.TOOL_ROUTER_URL ??
                "http://host.docker.internal:4100",
            memory_svc_url: process.env.MEMORY_SVC_URL ??
                "http://host.docker.internal:4101",
            messages_svc_url: process.env.MESSAGES_SVC_URL ??
                "http://host.docker.internal:4102",
            meta_svc_url: process.env.META_SVC_URL ??
