 ...
 */
export function extractTelegramName(
  ...
  if (from && typeof from === "object" && !Array.isArray(from)) {
    const name = buildNameFromObject(from as Record<string, unknown>);
    if (name) return name;
  }
...
function buildNameFromObject(
  obj: Record<string, unknown>,
): string | null {
  const firstName =
    typeof obj["first_name"] === "string" ? obj["first_name"].trim() : null;
  const lastName =
    typeof obj["last_name"] === "string" ? obj["last_name"].trim() : null;
  const username =
