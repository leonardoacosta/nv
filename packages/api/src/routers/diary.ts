 ...
 */
function normalizeToolsUsed(raw: unknown): ToolCallDetail[] {
  ...
  return raw.map((item) => {
    ...
    if (item && typeof item === "object") {
      ...
      return {
        ...
        input_summary:
          typeof obj["input_summary"] === "string" ? obj["input_summary"] : "",
        duration_ms:
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [channelResult] = await db
        .select({
          count: sql<number>`count(distinct ${diary.channel})::int`,
        })
        .from(diary)
        .where(where);
