...
export const obligationRouter = createTRPCRouter({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      const conditions = [];
      if (input.status) conditions.push(eq(obligations.status, input.status));
      if (input.owner) conditions.push(eq(obligations.owner, input.owner));
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.detected_action !== undefined) updates.detectedAction = input.detected_action;
      if (input.project_code !== undefined) updates.projectCode = input.project_code;
