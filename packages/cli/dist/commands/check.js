...
import { checkFleetDeep } from "../lib/fleet.js";
import { subheading, heading, check, cross, circle, padRight, gray, } from "../lib/format.js";
...
async function checkPostgres() {
    const { stdout: dbUrl, exitCode } = await exec("doppler", ["secrets", "get", "DATABASE_URL", "--plain", "--project", "nova", "--config", "prd"], 5000);
    if (exitCode !== 0 || !dbUrl) {
        return { connected: false, tableCount: null, rowSummary: null, error: "no DATABASE_URL" };
    }
    // Table count
    const { stdout: tableOut, exitCode: tblExit } = await exec("psql", [
        dbUrl, "-t", "-A", "-c",
    ...
    const { stdout: rowOut } = await exec("psql", [
        dbUrl, "-t", "-A", "-c",
        [
...
async function checkDoppler() {
    const { exitCode } = await exec("doppler", ["secrets", "--project", "nova", "--config", "prd", "--only-names"], 5000);
    return { configured: exitCode === 0 };
}
async function checkGraphTokens() {
    const tokens = [
    ...
    const results = await Promise.allSettled(tokens.map(async (t) => {
        ...
        const { stdout, exitCode } = await exec("ssh", ["-o", "ConnectTimeout=5", "-o", "BatchMode=yes", "cloudpc", "powershell", "-NoProfile", "-Command", psCmd], 15_000);
