...
import { heading, subheading, green, red, yellow, gray, padRight, } from "../lib/format.js";
...
export async function health() {
    ...
    console.log("");
    if (daemon.active) {
        console.log(`Daemon:     ${green("active")} (nova-ts.service)    uptime: ${daemon.uptime}`);
    }
    else {
    ...
    if (dashboard.running) {
        ...
        console.log(`Dashboard:  ${red(dashboard.status)} (nv-dashboard-1)`);
    }
    // Fleet
    ...
    if (channels.length > 0) {
        ...
        for (const ch of channels) {
            ...
            const status = ch.status === "connected"
                ? green("connected")
