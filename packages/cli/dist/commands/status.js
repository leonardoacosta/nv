...
export async function status() {
    ...
    const fleetColor = healthyCount === totalCount
        ? green
        : healthyCount > 0
            ? yellow
            : red;
    ...
    const channelStr = connectedChannels > 0
        ? green(`${connectedChannels}/${totalChannels}`)
        : red(`0/${totalChannels}`);
    console.log(`Nova: daemon=${daemonStr} fleet=${fleetStr} dashboard=${dashboardStr} channels=${channelStr}`);
}
