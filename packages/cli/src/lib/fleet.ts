...
async function runDeep(
  label: string,
  port: number,
  fn: () => Promise<{ status: "ok" | "empty"; detail: string }>,
  timeoutMs: number,
...
export async function checkFleetDeep(timeoutMs = 15_000): Promise<DeepCheckResult[]> {
  const results = await Promise.allSettled([
    ...
    runDeep("tool-router", 4100, async () => {
      const data = await fetchJson<{ services?: unknown[] }>(
    ...
    runDeep("channels-svc", 4103, async () => {
      const data = await fetchJson<{ channels?: Array<{ status?: string }> }>(
        "http://127.0.0.1:4103/channels",
        timeoutMs,
      );
      const channels = Array.isArray(data.channels) ? data.channels : [];
      if (channels.length === 0) return { status: "empty", detail: "no channels configured" };
    ...
    runDeep("discord-svc", 4104, async () => {
      const data = await fetchJson<{ guilds?: unknown[] }>(
        "http://127.0.0.1:4104/guilds",
        timeoutMs,
      );
    ...
    runDeep("schedule-svc", 4106, async () => {
      await fetchJson<unknown>("http://127.0.0.1:4106/health", timeoutMs);
    ...
    runDeep("azure-svc", 4109, async () => {
      ...
      if (data.output) {
        try {
          const parsed = JSON.parse(data.output) as { state?: string; name?: string };
          ...
          if (data.output.trim()) return { status: "ok", detail: "az: subscription active" };
        }
      }
