...
import { registerAzureTools } from "./tools/azure-cli.js";
...
// Register azure CLI tool
for (const tool of registerAzureTools(config)) {
    registry.register(tool);
}
logger.info({ tools: registry.list().map((t) => t.name) }, `Registered ${registry.list().length} tools`);
if (isMcpMode) {
    // MCP stdio transport
    ...
    const server = serve({ fetch: app.fetch, port: config.servicePort }, (info) => {
        logger.info({
