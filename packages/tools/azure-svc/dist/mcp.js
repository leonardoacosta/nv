...
export async function startMcpServer(config, logger) {
    ...
    server.registerTool("azure_cli", {
        ...
        inputSchema: z.object({
            command: z
                .string()
                .describe("The full Azure CLI command to run, starting with 'az'. " +
        ...
        try {
            ...
            logger.error({ command: command.slice(0, 80), error: errorMessage }, "MCP: azure_cli failed");
            return {
                content: [
                    {
                        type: "text",
