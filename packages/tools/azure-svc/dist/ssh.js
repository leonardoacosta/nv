 ...
 */
export function sshCloudPC(host, command) {
    ...
    return new Promise((resolve, reject) => {
        const child = execFile("ssh", [
            ...
            host,
            command,
        ], { timeout: DEFAULT_TIMEOUT_MS }, (error, stdout, stderr) => {
            const durationMs = Date.now() - startMs;
            if (error) {
                // Timeout: child_process sets error.killed when the process was killed by timeout
                if (error.killed || error.code === "ETIMEDOUT") {
                    log.error({ host, command: cmdPreview, durationMs, error: "timeout" }, `SSH timed out after ${Math.round(DEFAULT_TIMEOUT_MS / 1000)}s`);
                    return reject(new SshError(`CloudPC SSH timed out after ${Math.round(DEFAULT_TIMEOUT_MS / 1000)} seconds`, 504));
                }
                const stderrStr = stderr ?? "";
                // Connection failure
                if (UNREACHABLE_PATTERNS.some((pattern) => stderrStr.includes(pattern))) {
                    log.error({ host, command: cmdPreview, durationMs, error: "unreachable" }, "CloudPC unreachable");
                    return reject(new SshError("CloudPC unreachable -- cannot connect to 'cloudpc' via SSH", 503));
                }
                ...
                log.warn({ host, command: cmdPreview, durationMs, error: errMsg.slice(0, 500) }, "SSH command failed");
                return reject(new SshError(errMsg, 502));
            }
            // Filter noise lines from stdout
            const filtered = (stdout ?? "")
                .split("\n")
                .filter((line) => !NOISE_PATTERNS.some((pattern) => line.includes(pattern)))
                .join("\n")
                .trim();
            log.info({
                host,
        ...
        child.on("error", (err) => {
            const durationMs = Date.now() - startMs;
            log.error({ host, command: cmdPreview, durationMs, error: err.message }, "SSH child process error");
