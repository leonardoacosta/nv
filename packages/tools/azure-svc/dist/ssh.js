...
export function sshCloudPC(host, command) {
    return new Promise((resolve, reject) => {
        const child = execFile("ssh", ["-o", "ConnectTimeout=10", host, command], { timeout: 60_000 }, (error, stdout, stderr) => {
            if (error) {
                // Timeout: child_process sets error.killed when the process was killed by timeout
                if (error.killed || error.code === "ETIMEDOUT") {
                    return reject(new SshError("CloudPC SSH timed out after 60 seconds", 504));
                }
