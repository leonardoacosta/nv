...
function sanitizeCommand(command: string): string {
  ...
  if (!trimmed.startsWith("az ")) {
    throw new Error(
      'Command must start with "az ". Example: az vm list --resource-group myRG',
    );
  }
...
export async function runAzureCli(
  ...
  const hasOutputFlag =
    sanitizedArgs.includes("--output ") || sanitizedArgs.includes("-o ");
  ...
  return [
    {
      ...
      handler: async (input) => {
        ...
        if (typeof command !== "string" || !command.trim()) {
          throw new Error(
            "Missing required 'command' parameter. Example: az vm list",
          );
        }
