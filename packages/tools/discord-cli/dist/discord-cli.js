...
async function guildsCommand(client) {
...
var TEXT_CHANNEL = 0;
var CATEGORY_CHANNEL = 4;
async function channelsCommand(client, guildId) {
  ...
  for (const ch of textChannels) {
    const categoryName = ch.parent_id ? categoryMap.get(ch.parent_id) ?? UNCATEGORIZED : UNCATEGORIZED;
...
var USER_MESSAGE = 0;
async function messagesCommand(client, channelId, limit) {
...
program2.name("discord-cli").description("Discord bot CLI for read operations").version("0.1.0");
program2.command("guilds").description("List all guilds the bot is a member of").action(async () => {
  const client = new DiscordClient();
  await guildsCommand(client);
});
program2.command("channels <guild-id>").description("List text channels in a guild, grouped by category").action(async (guildId) => {
  const client = new DiscordClient();
  await channelsCommand(client, guildId);
});
program2.command("messages <channel-id>").description("Read recent messages from a channel").option("-l, --limit <number>", "Number of messages to fetch (max 100)", "50").action(async (channelId, options) => {
